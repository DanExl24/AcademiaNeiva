...
- **RN-CIE-004 (Ocultamiento de Alumnos Retirados):** El motor de generación de boletines en bloque omite de la cola de procesamiento PDF a todos los estudiantes cuya matrícula figure como `CANCELADA` en el año en curso, impidiendo que alumnos expulsados o retirados aparezcan en las impresiones oficiales del colegio.
...
| **Controller de Cierre Directivo** | [academicAdminController.ts](file:///c:/Users/alejo/Downloads/segundoProyecto/backend/src/controllers/academicAdminController.ts) — `closeAcademicPeriod`, `approveAcademicPeriod`, `reopenAcademicPeriod`, `reopenSubjectClosure`, `getPeriodClosureDetails`. |
