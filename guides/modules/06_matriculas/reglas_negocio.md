...
- **Descripción:** Una solicitud de tipo `EXTRAORDINARIA` se origina mediante autorización directiva expresa, bien sea respondiendo a un **Ticket de Soporte** (`tipo_incidencia = 'MATRICULA_EXTRAORDINARIA'`) o directamente desde la consola de **Gestión de Matrículas** (`EnrollmentManagement.vue` a través del modal `ExtraordinaryEnrollmentModal.vue`).
  1. **Validaciones Previas y Elegibilidad de Estudiante Existente:**
     ...
     - **Elegibilidad de Estudiante Existente vs. Reingreso:** Si se selecciona un estudiante registrado en la institución (`id_estudiante`), este debe encontrarse exclusivamente en estado **`INACTIVO`** (registro sin matrícula formal activa). Si el estudiante se encuentra en estado **`RETIRADO`**, el sistema bloquea su selección y orienta al directivo hacia el **Módulo de Reingreso de Estudiantes** (`ReingresoManagement.vue`) para auditar el motivo de retiro previo y aplicar la matriz documental inteligente. Si el alumno se encuentra **`ACTIVO`** (ya cuenta con matrícula vigente), **`SANCIONADO`** (presenta falta disciplinaria activa), **`EXPULSADO`** (inhabilitado permanentemente) o **`GRADUADO`** (ciclo culminado), el sistema bloquea su selección tanto en la interfaz modal como a nivel transaccional en el backend (`STUDENT_IS_RETIRED`, `STUDENT_ALREADY_ACTIVE`, `STUDENT_SANCTIONED`, `STUDENT_INELIGIBLE`).
