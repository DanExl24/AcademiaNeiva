    ...
    for (const group of groups) {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        const pUserRes = await client.query(
          ...
          [pEmail, parentHash, `PadreMock ${idx}`, `ApellidoP ${idx}`, group.id_colegio]
        );
        ...
        const pFamRes = await client.query(
          `INSERT INTO padre_familia (nombre, apellido, documeno, id_tipodocumento, id_colegio, id_usuario) VALUES ($1, $2, $3, 1, $4, $5) RETURNING id_padrefamilia`,
          [`PadreMock ${idx}`, `ApellidoP ${idx}`, `1000${idx}`, group.id_colegio, idPadreUser]
        );
        const idPadreFamilia = pFamRes.rows[0].id_padrefamilia;

        // 3. Crear Usuario Estudiante
        const sEmail = `estudiante${idx}@correo.com`;
        const sUserRes = await client.query(
          `INSERT INTO usuario (email, password, nombre, apellido, id_colegio, activo) VALUES ($1, $2, $3, $4, $5, true) RETURNING id_usuario`,
          [sEmail, studentHash, `EstudianteMock ${idx}`, `ApellidoS ${idx}`, group.id_colegio]
        );
        ...
        const estRes = await client.query(
          ...
          [`EstudianteMock ${idx}`, `ApellidoS ${idx}`, `2000${idx}`, `EST-MOCK-${idx}`, group.id_nivel, group.id_colegio, idEstUser]
        );
        ...
        await client.query(
          ...
          [idPadreFamilia, idEstudiante, group.id_colegio]
        );
        ...
        await client.query(
          ...
          [idEstudiante, group.id_nivel, group.id_colegio, yearId, pEmail, group.id_grupo]
        );
