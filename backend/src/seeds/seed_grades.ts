...
async function runSeedGrades() {
  ...
  try {
    ...
    for (const period of allPeriods) {
      ...
      for (const dg of detalleGradosDePeriodo) {
        ...
        const actRes = await client.query(
          ...
          [dg.id_detallegrado, period.id_periodo, dg.id_colegio, competenciaId]
        );
        ...
        for (const student of studentsInGroup) {
          ...
          if (isClosed) {
            await client.query(
              `INSERT INTO resultado_academico (id_estudiante, id_detallegrado, id_periodo, promedio, estado, fecha_cierre, id_docente, observacion)
               VALUES ($1, $2, $3, $4, $5, NOW(), $6, $7) ON CONFLICT DO NOTHING`,
              [student.id_estudiante, dg.id_detallegrado, period.id_periodo, nota, getEstado(nota), dg.id_docente, obsAcad.recomendaciones]
            );
          }
          ...
          await client.query(
            ...
            [student.id_estudiante, dg.id_detallegrado, period.id_periodo, obsAcad.fortalezas, obsAcad.debilidades, obsAcad.recomendaciones, dg.id_colegio]
          );
          ...
          if (Math.random() < 0.3) {
            ...
            await client.query(
              `INSERT INTO observacion_estudiante (id_estudiante, id_detallegrado, id_periodo, fortalezas, debilidades, recomendaciones, fecha, id_colegio, tipo)
               VALUES ($1, $2, $3, $4, $5, $6, NOW(), $7, 'CONVIVENCIA')`,
              [student.id_estudiante, dg.id_detallegrado, period.id_periodo, convObs.fortalezas, convObs.debilidades, convObs.recomendaciones, dg.id_colegio]
            );
          ...
          if (Math.random() < 0.15) {
            ...
            await client.query(
              `INSERT INTO observacion_estudiante (id_estudiante, id_detallegrado, id_periodo, fortalezas, debilidades, recomendaciones, fecha, id_colegio, tipo)
               VALUES ($1, $2, $3, $4, $5, $6, NOW(), $7, 'GENERAL')`,
              [student.id_estudiante, dg.id_detallegrado, period.id_periodo, genObs.fortalezas, genObs.debilidades, genObs.recomendaciones, dg.id_colegio]
            );
    ...
    const failCount = await pool.query(`SELECT COUNT(*) as total FROM resultado_academico WHERE promedio < 3.0`);
    const passCount = await pool.query(`SELECT COUNT(*) as total FROM resultado_academico WHERE promedio >= 3.0`);
