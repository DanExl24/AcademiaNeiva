...
export const getParentsManagementData = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const nivelesRes = await db
      ...
      .select([sql<number>`MIN(id_nivel)`.as("id_nivel"), "nombre"])
      .where((eb) => eb.or([eb("id_colegio", "=", id_colegio), eb("id_colegio", "is", null)]))
      .groupBy("nombre")
      .orderBy(sql`MIN(id_nivel)`, "asc")
      .execute();

    const gradosRes = await db
      .selectFrom("tipo_grado")
      .select([sql<number>`MIN(id_tipo_grado)`.as("id_tipo_grado"), "nombre"])
      .groupBy("nombre")
      .orderBy(sql`MIN(id_tipo_grado)`, "asc")
      .execute();
...
export const updateParent = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await db.transaction().execute(async (trx) => {
      ...
      if (!oldParent) {
        throw new Error("PADRE_NOT_FOUND");
      }
      ...
      if (isSupervision && finalDocumento && finalDocumento !== oldParent.documento) {
        await validateDocumentUniqueness(trx, finalDocumento, "acudiente", { excludeUsuarioId: oldParent.id_usuario }, finalTipoDoc);
      }
      ...
      const updatedParent = await trx
        ...
        .where("id_padrefamilia", "=", parentId)
        .returningAll()
      ...
      if (oldParent.id_usuario) {
        await trx
          ...
          .set({
            ...
            id_tipodocumento: finalTipoDoc,
            email: finalEmail
          })
          .where("id_usuario", "=", oldParent.id_usuario)
          .execute();
      }
      ...
      if (activeAuditoriaId) {
        ...
        await trx
          ...
          .values({
            ...
            modulo: "PADRES_FAMILIA",
            tipo_accion: "MODIFICACION",
            accion: "Actualizacion de datos del padre de familia",
            recurso_afectado: `Padre ID: ${parentId} (${nombre} ${apellido})`,
            id_usuario_afectado: null,
            valor_antiguo: JSON.stringify({ nombre: oldParent.nombre, apellido: oldParent.apellido, documento: oldParent.documento }),
            valor_nuevo: JSON.stringify({ nombre: nombre.trim(), apellido: apellido.trim(), documento: documento.trim() }),
            motivo_cambio: "Actualizacion de datos del acudiente"
          })
          .execute();
      }
    ...
    if (error.message === "PADRE_NOT_FOUND") {
      res.status(404).json({ error: "Padre de familia no encontrado" });
      return;
    }
...
export const updateParentAccountStatus = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const userId = parent.id_usuario;
    ...
    await db.transaction().execute(async (trx) => {
      await trx
        ...
        .set({
          activo: activo,
          estado: activo ? ("ACTIVO" as any) : ("SUSPENDIDO" as any),
          logged_out_at: activo ? undefined : sql`NOW()`
        })
        .where("id_usuario", "=", userId)
        .execute();

      await sql`UPDATE padre_familia SET estado = ${activo ? "ACTIVO" : "INACTIVO"} WHERE id_padrefamilia = ${parentId}`.execute(trx);
    });

    res.json({
      message: `La cuenta de usuario de ${parent.nombre} ${parent.apellido} ha sido ${activo ? "activada" : "inactivada"} exitosamente.`,
      activo
    });
  } catch (error: any) {
