        ...
        WHERE (pf.id_colegio = $1 OR u.id_colegio = $1)
          AND (
            ...
            NOT EXISTS (
              ...
              WHERE al.id_anio = $2
                AND (
                  EXTRACT(YEAR FROM COALESCE(pf.fecha_creacion, u.fecha_creacion)) > NULLIF(regexp_replace(al.calendario, '\D', '', 'g'), '')::int
                  OR (al.fecha_fin IS NOT NULL AND DATE(COALESCE(pf.fecha_creacion, u.fecha_creacion)) > al.fecha_fin)
                )
            )
          )
