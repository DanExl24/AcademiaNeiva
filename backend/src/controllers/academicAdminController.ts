         ...
         WHERE d.id_colegio = $1
            AND (
              ...
              NOT EXISTS (
                ...
                WHERE al.id_anio = $2
                  AND (
                    EXTRACT(YEAR FROM COALESCE(d.fecha_creacion, u.fecha_creacion)) > NULLIF(regexp_replace(al.calendario, '\D', '', 'g'), '')::int
                    OR (al.fecha_fin IS NOT NULL AND DATE(COALESCE(d.fecha_creacion, u.fecha_creacion)) > al.fecha_fin)
                  )
              )
            )
