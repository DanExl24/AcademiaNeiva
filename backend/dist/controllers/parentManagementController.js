 ...
 */
const getParentsManagementData = async (req, res) => {
    const { schoolId } = req.params;
    const { busqueda, estadoCuenta, alertaHijo, id_nivel, id_tipo_grado, cantHijos, estadoMatricula, yearId } = req.query;
    ...
    try {
        ...
        const params = [id_colegio, selectedYearId];
        let whereClauses = [];
    ...
    `;
        ...
        const nivelesRes = await db_1.pool.query(`SELECT MIN(id_nivel) AS id_nivel, nombre
       FROM nivel_escolar
       WHERE id_colegio = $1 OR id_colegio IS NULL
       GROUP BY nombre
       ORDER BY MIN(id_nivel) ASC`, [id_colegio]);
        const gradosRes = await db_1.pool.query(`SELECT MIN(id_tipo_grado) AS id_tipo_grado, nombre
       FROM tipo_grado
       GROUP BY nombre
...
const getParentDetail = async (req, res) => {
    ...
    try {
        const parentRes = await db_1.pool.query(`SELECT
         pf.id_padrefamilia,
         pf.nombre,
         pf.apellido,
         u.documento,
       ...
       ORDER BY e.apellido ASC, e.nombre ASC`, [parentId, selectedYearId]);
        ...
        const childrenWithStats = await Promise.all(children.map(async (child) => {
            ...
            if (child.id_grupo && child.id_anio) {
                const avgRes = await db_1.pool.query(`SELECT ROUND(AVG(ra.promedio)::numeric, 2) AS promedio
        ...
        const tiposDocRes = await db_1.pool.query("SELECT id_tipodocumento, tipo FROM tipo_documento ORDER BY id_tipodocumento ASC");
        res.json({
            parent,
...
const getDocumentTypes = async (req, res) => {
    try {
        const result = await db_1.pool.query("SELECT id_tipodocumento, tipo FROM tipo_documento ORDER BY id_tipodocumento ASC");
 ...
 * Body: { activo: boolean }
 */
const updateParentAccountStatus = async (req, res) => {
    const { id } = req.params;
    const parentId = parseInt(id);
    ...
    try {
        const parentRes = await db_1.pool.query(`SELECT pf.id_padrefamilia, pf.id_usuario, pf.id_colegio, pf.nombre, pf.apellido, u.email
       FROM padre_familia pf
       ...
       WHERE id_usuario = $2`, [activo, userId]);
        await db_1.pool.query(`UPDATE padre_familia
       SET estado = CASE WHEN $1 = TRUE THEN 'ACTIVO' ELSE 'INACTIVO' END
       WHERE id_padrefamilia = $2`, [activo, parentId]);
        res.json({
