...
const validatePeriodClosed = async (req, res) => {
    ...
    try {
        const periodRes = await kysely_1.db
...
const getStudentBoletin = async (req, res) => {
    ...
    try {
        ...
        const periodoDetails = await kysely_1.db
            .selectFrom("periodo_academico")
        ...
        const studentInfo = await kysely_1.db
        ...
        const materiasRes = await kysely_1.db
        ...
        const studentGroupSubquery = kysely_1.db
        ...
        const notasRes = await kysely_1.db
            ...
            .on((0, kysely_2.sql) `COALESCE(ra.promedio, calc.promedio_calculado)`, ">=", kysely_2.sql.ref("ev.valor_minimo"))
            .on((0, kysely_2.sql) `COALESCE(ra.promedio, calc.promedio_calculado)`, "<=", kysely_2.sql.ref("ev.valor_maximo")))
            .select([
            "gm.id_materia",
        ...
        const obsRes = await kysely_1.db
        ...
        const compRes = await kysely_1.db
        ...
        const materias = materiasRes.map(m => {
            ...
            const desempenos = Array.from(new Set(compRes.filter(c => Number(c.id_materia) === mId).map(c => c.descripcion))).slice(0, 3);
            return {
                materia: m.materia,
        ...
        const groupAveragesCte = kysely_1.db
            ...
            .select([
            "m2.id_estudiante",
            (0, kysely_2.sql) `ROUND(AVG(COALESCE(ra2.promedio, calc.promedio_calculado, 0))::numeric, 2)`.as("student_avg")
        ])
            .where("m2.id_grupo", "=", kysely_1.db.selectFrom("matricula").select("id_grupo").where("id_estudiante", "=", numEstudiante).where("id_anio", "=", idAnio).limit(1))
            .where("m2.estado", "=", 'ACTIVA')
            .groupBy("m2.id_estudiante");
        const rankedCte = kysely_1.db
            .with("group_averages", () => groupAveragesCte)
            .selectFrom("group_averages")
            .select([
            "id_estudiante",
            "student_avg",
            (0, kysely_2.sql) `RANK() OVER (ORDER BY student_avg DESC)`.as("puesto"),
            (0, kysely_2.sql) `COUNT(*) OVER ()`.as("total_grupo")
        ]);
        ...
        const escalaRows = await kysely_1.db
        ...
        if (escalaRows.length > 0) {
            const matchedLevel = escalaRows.find((e) => promedioGlobal >= parseFloat(e.valor_minimo) && promedioGlobal <= parseFloat(e.valor_maximo));
            nivelDesempeno = matchedLevel?.nivel || escalaRows[escalaRows.length - 1]?.nivel || 'Sin datos';
        }
        // 11. Fetch Firmas (Titular y Rector)
        const titularRow = await kysely_1.db
            .selectFrom("docente as d")
            .innerJoin("grupos as g", "g.id_docente", "d.id_docente")
            .select((0, kysely_2.sql) `d.nombre || ' ' || d.apellido`.as("nombre_completo"))
            .where("g.id_grupo", "=", kysely_1.db.selectFrom("matricula").select("id_grupo").where("id_estudiante", "=", numEstudiante).where("id_anio", "=", idAnio).limit(1))
            .executeTakeFirst();
        const rectorRow = await kysely_1.db
            .selectFrom("directivo as d")
            .innerJoin("usuario as u", "u.id_usuario", "d.id_usuario")
            .select((0, kysely_2.sql) `u.nombre || ' ' || u.apellido`.as("nombre_completo"))
...
const getGradeBoletines = async (req, res) => {
    ...
    try {
        ...
        const studentsRes = await kysely_1.db
            .selectFrom("matricula")
