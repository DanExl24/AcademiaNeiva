...
const getStudentBoletin = async (req, res) => {
    ...
    `, [id_estudiante]);
        ...
        const notasRes = await db_1.pool.query(`
      SELECT 
        dg.id_materia,
    ...
    `, [id_estudiante, idAnio]);
        ...
        const obsRes = await db_1.pool.query(`
    ...
    `, [id_estudiante, id_periodo]);
        ...
        const compRes = await db_1.pool.query(`
    ...
    `, [id_estudiante, id_periodo]);
        ...
        const ausenciasRes = await db_1.pool.query(`
    ...
    `, [id_estudiante, id_periodo]);
        ...
        const materias = materiasRes.rows.map(m => {
            ...
            const notas = notasRes.rows.filter(n => Number(n.id_materia) === mId &&
        ...
        const currentPeriodGrades = notasRes.rows.filter(n => n.id_periodo === parseInt(id_periodo, 10));
    ...
    `, [id_estudiante, id_periodo]);
        ...
        const escalaRes = await db_1.pool.query(`
    ...
    `, [studentInfo.id_colegio]);
        ...
        const firmasRes = await db_1.pool.query(`
      SELECT 
