...
exports.actualizarColegio = actualizarColegio;
/**
 * POST /admin/colegios/upload-escudo
 * Subir archivo del escudo del colegio.
 */
const uploadEscudo = async (req, res) => {
    try {
        if (!req.file) {
...
exports.uploadEscudo = uploadEscudo;
/**
 ...
 */
const cambiarEstadoColegio = async (req, res) => {
    ...
    try {
        ...
        const colegioActual = await client.query('SELECT id_colegio, nombre, estado FROM colegio WHERE id_colegio = $1', [id]);
        ...
        await client.query(`UPDATE colegio SET ${updateFields.join(', ')} WHERE id_colegio = $${updateParams.length}`, updateParams);
       ...
       WHERE d.id_colegio = $1 AND d.estado = 'ACTIVO'`, [id]);
        // Crear notificaciones en BD para cada directivo
        for (const dir of directivos.rows) {
            await client.query(`INSERT INTO notificacion_colegio (id_colegio, id_directivo, tipo, mensaje, estado_anterior, estado_nuevo)
         VALUES ($1, $2, 'CAMBIO_ESTADO', $3, $4, $5)`, [id, dir.id, `El estado del colegio ${colegioNombre} cambió de ${estadoAnterior} a ${estado}`, estadoAnterior, estado]);
            // Email solo cuando el colegio es SUSPENDIDO
            if (estado === 'SUSPENDIDO') {
                adminGeneralNotificationService_1.AdminGeneralNotificationService.sendColegioSuspendido(dir.email, `${dir.nombre} ${dir.apellido || ''}`.trim(), colegioNombre, motivo || 'No especificado');
            }
        }
        // Si se elimina el colegio, desvincular usuarios
        if (estado === 'ELIMINADO') {
            await client.query('UPDATE usuario SET id_colegio = NULL WHERE id_colegio = $1', [id]);
        }
...
exports.cambiarEstadoColegio = cambiarEstadoColegio;
/**
...
const eliminarColegio = async (req, res) => {
    ...
    return (0, exports.cambiarEstadoColegio)(req, res);
};
exports.eliminarColegio = eliminarColegio;
// ═══════════════════════════════════════════════════════════════════════════
// GESTIÓN DE USUARIOS
// ═══════════════════════════════════════════════════════════════════════════
/**
...
const cambiarEstadoUsuario = async (req, res) => {
    try {
        ...
        params.push(id);
        const result = await db_1.pool.query(`UPDATE usuario SET ${updateFields.join(', ')} WHERE id_usuario = $${params.length} RETURNING id_usuario, email, nombre, estado`, params);
...
const restablecerPassword = async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query('UPDATE usuario SET password = $1 WHERE id_usuario = $2 RETURNING id_usuario, email, nombre', [hashedPassword, id]);
        if (result.rows.length === 0) {
            res.status(404).json({ error: 'Usuario no encontrado' });
            return;
        }
...
const forzarCierreSesion = async (req, res) => {
    try {
        ...
        const user = await db_1.pool.query('SELECT id_usuario, email, nombre FROM usuario WHERE id_usuario = $1', [id]);
        if (user.rows.length === 0) {
            res.status(404).json({ error: 'Usuario no encontrado' });
            return;
        }
        // Actualizar logged_out_at para invalidar todos los tokens emitidos antes de ahora
        await db_1.pool.query('UPDATE usuario SET logged_out_at = NOW() WHERE id_usuario = $1', [id]);
 ...
 */
const listarDirectivos = async (req, res) => {
    try {
        const { colegioId } = req.params;
        const result = await db_1.pool.query(`SELECT d.id, d.cargo, d.estado, d.fecha_vinculacion, d.fecha_desvinculacion,
              u.id_usuario, u.nombre, u.apellido, u.email
       FROM directivo d
       JOIN usuario u ON d.id_usuario = u.id_usuario
       WHERE d.id_colegio = $1
       ORDER BY d.fecha_vinculacion DESC`, [colegioId]);
...
exports.registrarDirectivo = registrarDirectivo;
/**
...
const actualizarDirectivo = async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query('UPDATE directivo SET cargo = COALESCE($1, cargo) WHERE id = $2 RETURNING *', [cargo, id]);
        if (result.rows.length === 0) {
...
exports.actualizarDirectivo = actualizarDirectivo;
/**
 ...
 */
const solicitarSupervision = async (req, res) => {
    const client = await db_1.pool.connect();
    try {
       ...
       WHERE clave IN ('supervision_duracion_minima_minutos', 'supervision_duracion_maxima_minutos')`);
        ...
        const activa = await client.query(`SELECT id_auditoria FROM auditoria_supervision
       WHERE id_admin_general = $1 AND id_colegio = $2
         AND estado_supervision IN ('SOLICITADA', 'APROBADA', 'ACTIVA')
         AND eliminado = FALSE`, [req.user.id, id_colegio]);
       ...
       FROM directivo d
       JOIN usuario u ON d.id_usuario = u.id_usuario
       WHERE d.id_colegio = $1 AND d.estado = 'ACTIVO'`, [id_colegio]);
        ...
        for (const dir of directivos.rows) {
         ...
         VALUES ($1, $2, 'SOLICITUD', $3)`, [result.rows[0].id_auditoria, dir.id,
                `El Admin General ha solicitado entrar en modo supervisión (${tipo_supervision}) al colegio ${colegioNombre}`]);
            // Email
            adminGeneralNotificationService_1.AdminGeneralNotificationService.sendSupervisionSolicitada(dir.email, `${dir.nombre} ${dir.apellido || ''}`.trim(), adminNombre, colegioNombre, motivo, tipo_supervision);
        }
        await client.query('COMMIT');
        res.status(201).json(result.rows[0]);
    }
...
exports.solicitarSupervision = solicitarSupervision;
/**
 * POST /admin/supervision/:id/aprobar
 * Aprobar una solicitud de supervisión (ejecutado por un directivo).
 */
const aprobarSupervision = async (req, res) => {
    const client = await db_1.pool.connect();
    try {
        const { id } = req.params;
        await client.query('BEGIN');
        // Obtener la auditoría
        const auditoria = await client.query(`SELECT a.*, c.nombre AS colegio_nombre, u.email AS admin_email, u.nombre AS admin_nombre
       FROM auditoria_supervision a
       JOIN colegio c ON c.id_colegio = a.id_colegio
       JOIN usuario u ON u.id_usuario = a.id_admin_general
       WHERE a.id_auditoria = $1 AND a.estado_supervision = 'SOLICITADA' AND a.eliminado = FALSE`, [id]);
        ...
        const directivo = await client.query(`SELECT d.id FROM directivo d
       WHERE d.id_usuario = $1 AND d.id_colegio = $2 AND d.estado = 'ACTIVO'`, [req.user.id, auditoria.rows[0].id_colegio]);
       ...
       WHERE id_auditoria = $2`, [directivo.rows[0].id, id]);
        ...
        adminGeneralNotificationService_1.AdminGeneralNotificationService.sendSupervisionAprobada(aud.admin_email, aud.admin_nombre, aud.colegio_nombre, `${req.user.email}`, aud.tipo_supervision, aud.duracion_maxima_minutos);
        await client.query('COMMIT');
...
exports.aprobarSupervision = aprobarSupervision;
/**
 ...
 */
const entrarSupervision = async (req, res) => {
    const client = await db_1.pool.connect();
    try {
        const { id } = req.params;
        ...
        const validPassword = await bcrypt_1.default.compare(password, userRes.rows[0].password);
        if (!validPassword) {
            await client.query('ROLLBACK');
        ...
        // Obtener auditoría
        const auditoria = await client.query(`SELECT a.*, c.nombre AS colegio_nombre
       FROM auditoria_supervision a
       JOIN colegio c ON c.id_colegio = a.id_colegio
       WHERE a.id_auditoria = $1 AND a.estado_supervision = 'APROBADA'
         AND a.id_admin_general = $2 AND a.eliminado = FALSE`, [id, req.user.id]);
        if (auditoria.rows.length === 0) {
            await client.query('ROLLBACK');
       ...
       SET estado_supervision = 'ACTIVA',
           fecha_entrada = NOW(),
           motivo_entrada = $1
       WHERE id_auditoria = $2`, [motivo_entrada || auditoria.rows[0].motivo_solicitud, id]);
        const aud = auditoria.rows[0];
        // Notificar a directivos del colegio
        const directivos = await client.query(`SELECT d.id, u.email, u.nombre, u.apellido
       FROM directivo d
       JOIN usuario u ON d.id_usuario = u.id_usuario
       WHERE d.id_colegio = $1 AND d.estado = 'ACTIVO'`, [aud.id_colegio]);
        for (const dir of directivos.rows) {
            await client.query(`INSERT INTO notificacion_supervision (id_auditoria, id_directivo, tipo_notificacion, mensaje)
         VALUES ($1, $2, 'ENTRADA', $3)`, [id, dir.id, `El Admin General ha ENTRADO en modo supervisión al colegio ${aud.colegio_nombre}`]);
            adminGeneralNotificationService_1.AdminGeneralNotificationService.sendSupervisionIniciada(dir.email, `${dir.nombre} ${dir.apellido || ''}`.trim(), req.user.email, aud.colegio_nombre, aud.tipo_supervision);
        }
        await client.query('COMMIT');
        res.json({
...
const salirSupervision = async (req, res) => {
    ...
    try {
       ...
       SET estado_supervision = 'FINALIZADA',
           fecha_salida = NOW()
       WHERE id_auditoria = $1`, [id]);
        const aud = auditoria.rows[0];
        // Contar acciones realizadas
        const acciones = await client.query('SELECT COUNT(*) AS total FROM auditoria_acciones_realizadas WHERE id_auditoria = $1', [id]);
        // Calcular duración
       ...
       WHERE d.id_colegio = $1 AND d.estado = 'ACTIVO'`, [aud.id_colegio]);
        for (const dir of directivos.rows) {
            await client.query(`INSERT INTO notificacion_supervision (id_auditoria, id_directivo, tipo_notificacion, mensaje)
         VALUES ($1, $2, 'SALIDA', $3)`, [id, dir.id, `El Admin General ha SALIDO del modo supervisión del colegio ${aud.colegio_nombre}. Duración: ${duracionStr}. Acciones: ${acciones.rows[0].total}`]);
            adminGeneralNotificationService_1.AdminGeneralNotificationService.sendSupervisionFinalizada(dir.email, `${dir.nombre} ${dir.apellido || ''}`.trim(), req.user.email, aud.colegio_nombre, duracionStr, parseInt(acciones.rows[0].total));
        }
        await client.query('COMMIT');
        res.json({
 ...
 */
const verAccionesSupervision = async (req, res) => {
    try {
        const { id } = req.params;
...
exports.verAccionesSupervision = verAccionesSupervision;
/**
 * GET /admin/supervision/historial
 * Historial de supervisiones con filtros.
 */
const historialSupervision = async (req, res) => {
    try {
...
exports.historialSupervision = historialSupervision;
/**
...
const exportarAuditoria = async (req, res) => {
    ...
    try {
       ...
       FROM auditoria_supervision a
       JOIN colegio c ON c.id_colegio = a.id_colegio
       WHERE a.id_auditoria = $1 AND a.eliminado = FALSE`, [id]);
        if (auditoria.rows.length === 0) {
            await client.query('ROLLBACK');
       ...
       VALUES ($1, 'AUDITORIA', 'EXPORTACION', 'Exportación de datos de auditoría', $2)`, [id, `Auditoría #${id} - ${auditoria.rows[0].colegio_nombre}`]);
 ...
 */
const registrarAccionAuditoria = async (idAuditoria, modulo, tipoAccion, accion, recursoAfectado, idUsuarioAfectado, valorAntiguo, valorNuevo, motivoCambio) => {
...
const obtenerStatsDashboard = async (req, res) => {
    try {
        // 1. Colegios KPIs
        const colegiosRes = await db_1.pool.query(`
      SELECT 
        COUNT(*)::int as total,
    ...
    `);
        ...
        // 2. Usuarios KPIs
        const usuariosRes = await db_1.pool.query(`
      SELECT 
        COUNT(*)::int as total,
        COUNT(CASE WHEN estado = 'ACTIVO' THEN 1 END)::int as activos
      FROM usuario
    ...
    `);
        ...
        const distribucionRes = await db_1.pool.query(`
    ...
    `);
        ...
        distribucionRes.rows.forEach((row) => {
        ...
        const actividadRes = await db_1.pool.query(`
      ...
      LIMIT 10
    `);
        let actividad = actividadRes.rows.map((row) => {
        ...
        const supervisionesRes = await db_1.pool.query(`
      SELECT 
    ...
    `);
        ...
        const auditoriasMesRes = await db_1.pool.query(`
      SELECT 
    ...
    `);
        ...
        let correosStatus = '🟢';
        try {
            const isSmtpHealthy = await adminGeneralNotificationService_1.AdminGeneralNotificationService.verifySMTP();
...
exports.obtenerStatsDashboard = obtenerStatsDashboard;
/**
 * GET /admin/auditorias
 * Listar todas las acciones de auditoría globalmente con filtros.
 */
const listarAuditoriasAcciones = async (req, res) => {
    try {
...
exports.listarAuditoriasAcciones = listarAuditoriasAcciones;
/**
