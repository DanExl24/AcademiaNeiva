...
const createTicket = async (req, res) => {
    ...
    try {
       ...
       RETURNING id_ticket`, [finalUserId, finalSenderName, finalSenderEmail, telefono || null, tipo_incidencia, asunto, descripcion, finalSchoolId, ticketStatus, id_estudiante ? Number(id_estudiante) : null]);
...
const updateTicketStatus = async (req, res) => {
    ...
    try {
        ...
        if (oldEstado !== 'EN_PROCESO' && estado === 'EN_PROCESO') {
            notificationService_1.NotificationService.sendReingresoInProcessEmail(ticket.correo_remitente, ticket.nombre_remitente, ticket.codigo_ticket || `TKT-${ticket.id_ticket}`, ticket.estudiante_nombre ? `${ticket.estudiante_nombre} ${ticket.estudiante_apellido || ''}`.trim() : undefined).catch((err) => console.error('Error enviando correo de ticket en proceso:', err));
        }
...
const getTicketByCode = async (req, res) => {
    ...
    try {
        const ticketRes = await db_1.pool.query(`SELECT t.*, 
              c.nombre AS colegio_nombre,
              e.nombre AS estudiante_nombre,
              e.apellido AS estudiante_apellido,
              u_e.documento AS estudiante_documento,
...
const addTicketObservation = async (req, res) => {
    ...
    try {
        ...
        const ticketRes = await db_1.pool.query('SELECT * FROM tickets_soporte WHERE id_ticket = $1', [id]);
        if (ticketRes.rows.length === 0) {
            return res.status(404).json({ error: 'Ticket no encontrado.' });
        }
        const ticket = ticketRes.rows[0];
        ...
        if (isStaff) {
            ...
            const userRes = await db_1.pool.query('SELECT email, id_colegio FROM usuario WHERE id_usuario = $1', [user.id]);
