...
exports.addVisitorObservation = exports.addTicketObservation = exports.getTicketByCode = exports.escalateTicket = exports.updateTicketStatus = exports.getTickets = exports.createTicket = void 0;
...
function parseBase36(str) {
    ...
    for (const char of str.toUpperCase()) {
        ...
        if (val < 0n)
            throw new Error("Carácter inválido en Base36");
        result = result * 36n + val;
    }
    return result;
}
// Helper: Codificar Código de Ticket Base36 Ofuscado
function encodeTicketCode(idTicket, idColegio, documento) {
...
const createTicket = async (req, res) => {
    ...
    try {
        ...
        let finalSchoolId = id_colegio ? Number(id_colegio) : null;
        ...
        if (user) {
            ...
            if (userRes.rows.length > 0) {
                ...
                if (userRole === 'DOCENTE') {
                    const docRes = await db_1.pool.query('SELECT documento FROM docente WHERE id_usuario = $1', [finalUserId]);
                    if (docRes.rows.length > 0)
                        userDocument = docRes.rows[0].documento;
                }
                else if (userRole === 'PADRE') {
                    const docRes = await db_1.pool.query('SELECT documento FROM padre_familia WHERE id_usuario = $1', [finalUserId]);
                    if (docRes.rows.length > 0)
                        userDocument = docRes.rows[0].documento;
                }
                else if (userRole === 'ESTUDIANTE') {
                    const docRes = await db_1.pool.query('SELECT documento FROM estudiante WHERE id_usuario = $1', [finalUserId]);
                    if (docRes.rows.length > 0)
                        userDocument = docRes.rows[0].documento;
                }
        ...
        const insertRes = await db_1.pool.query(`INSERT INTO tickets_soporte 
       (id_usuario, nombre_remitente, correo_remitente, telefono, tipo_incidencia, asunto, descripcion, id_colegio, estado)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING id_ticket`, [finalUserId, finalSenderName, finalSenderEmail, telefono || null, tipo_incidencia, asunto, descripcion, finalSchoolId, ticketStatus]);
...
const getTickets = async (req, res) => {
    ...
    try {
        ...
        if (userRole === 'DIRECTIVO') {
            const userRes = await db_1.pool.query('SELECT id_colegio FROM usuario WHERE id_usuario = $1', [user.id]);
...
const updateTicketStatus = async (req, res) => {
    ...
    try {
        const userRole = (user.role || '').toUpperCase();
        const ticketRes = await db_1.pool.query('SELECT id_colegio, fecha_escalado, estado, observaciones FROM tickets_soporte WHERE id_ticket = $1', [id]);
        ...
        if (oldEstado !== estado) {
            const authorRes = await db_1.pool.query('SELECT nombre, apellido FROM usuario WHERE id_usuario = $1', [user.id]);
            ...
            await db_1.pool.query('UPDATE tickets_soporte SET estado = $1, observaciones = $2 WHERE id_ticket = $3', [estado, JSON.stringify(currentObs), id]);
        }
        else {
            await db_1.pool.query('UPDATE tickets_soporte SET estado = $1 WHERE id_ticket = $2', [estado, id]);
        }
...
const escalateTicket = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const ticketRes = await client.query('SELECT estado, observaciones, id_colegio FROM tickets_soporte WHERE id_ticket = $1 FOR UPDATE', [id]);
            if (ticketRes.rows.length === 0) {
         ...
         WHERE id_ticket = $3`, [nuevoEstado, JSON.stringify(currentObs), id]);
...
const getTicketByCode = async (req, res) => {
    ...
    try {
        const ticketRes = await db_1.pool.query(`SELECT t.*, c.nombre as colegio_nombre 
       FROM tickets_soporte t 
       LEFT JOIN colegio c ON t.id_colegio = c.id_colegio 
       WHERE t.codigo_ticket = $1`, [code.trim().toUpperCase()]);
        if (ticketRes.rows.length === 0) {
...
const addTicketObservation = async (req, res) => {
    const user = req.user;
    const { id } = req.params;
    ...
    try {
        ...
        const ticketRes = await db_1.pool.query('SELECT observaciones, id_colegio, estado FROM tickets_soporte WHERE id_ticket = $1', [id]);
        const ticket = ticketRes.rows[0];
        ...
        await db_1.pool.query('UPDATE tickets_soporte SET observaciones = $1, estado = $2 WHERE id_ticket = $3', [JSON.stringify(currentObs), nuevoEstado, id]);
        return res.json({
...
exports.addTicketObservation = addTicketObservation;
// POST: Registrar observación de visitante (docente/padre) con reglas de ping-pong
const addVisitorObservation = async (req, res) => {
    ...
    try {
        const ticketRes = await db_1.pool.query('SELECT id_ticket, estado, observaciones, nombre_remitente FROM tickets_soporte WHERE codigo_ticket = $1', [codeStr.trim().toUpperCase()]);
        if (ticketRes.rows.length === 0) {
            return res.status(404).json({ error: 'Ticket no encontrado.' });
        }
        const ticket = ticketRes.rows[0];
        ...
        // Guardar en la BD tanto observaciones como el nuevo estado
        await db_1.pool.query('UPDATE tickets_soporte SET observaciones = $1, estado = $2 WHERE id_ticket = $3', [JSON.stringify(currentObs), nuevoEstado, ticket.id_ticket]);
        return res.json({
...
exports.addVisitorObservation = addVisitorObservation;
