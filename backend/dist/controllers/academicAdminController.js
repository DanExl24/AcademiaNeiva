...
const ensureAcademicYearForSchool = async (schoolId) => {
    const existing = await db_1.pool.query(`SELECT id_anio
     FROM anio_lectivo
    ...
    const fallback = await db_1.pool.query(`SELECT id_anio
     FROM anio_lectivo
     WHERE id_colegio = $1
     ORDER BY id_anio DESC
     LIMIT 1`, [schoolId]);
    ...
    const created = await db_1.pool.query(`INSERT INTO anio_lectivo (calendario, id_colegio, tipo_calendario, estado)
     VALUES ($1, $2, 'A', 'ABIERTO')
     RETURNING id_anio`, [String(currentYear), schoolId]);
...
const createAcademicPeriod = async (req, res) => {
    ...
    try {
       ...
       FROM periodo_academico
       WHERE id_colegio = $1 AND id_anio = $2`, [schoolId, finalYearId]);
       ...
       WHERE id_colegio = $1 AND id_anio = $2`, [schoolId, finalYearId]);
        ...
        const created = await db_1.pool.query(`INSERT INTO periodo_academico (nombre, estado, porcentaje, mes_inicio, dia_inicio, mes_fin, dia_fin, id_anio, id_colegio, trimestre)
       VALUES ($1, $2::estado_periodo, $3, $4, $5, $6, $7, $8, $9, $10)
       RETURNING id_periodo, nombre, estado, porcentaje, mes_inicio, dia_inicio, mes_fin, dia_fin, id_anio, trimestre`, [nombre, estado, porcentaje, mesInicio, diaInicio, mesFin, diaFin, finalYearId, schoolId, nextTrimestre]);
...
const deleteAcademicYear = async (req, res) => {
    ...
    try {
       ...
       WHERE id_anio = $1 AND id_colegio = $2`, [yearId, schoolId]);
        // Delete the year
        await client.query(`DELETE FROM anio_lectivo
       WHERE id_anio = $1 AND id_colegio = $2`, [yearId, schoolId]);
        await client.query("COMMIT");
...
const upsertCompetencyByAdmin = async (req, res) => {
    ...
    try {
        const contextRes = await db_1.pool.query(`SELECT p.id_anio, p.estado
       FROM periodo_academico p
       WHERE p.id_periodo = $1
        ...
        try {
            ...
            await client.query("BEGIN");
            const created = await (0, competencyMigration_1.syncCompetencyAcrossGrade)(client, context, periodId, descripcion, undefined, idDimension);
           ...
           FROM competencias 
           WHERE id_colegio = $1 
             AND (id_competencia = $2 OR (sync_uuid IS NOT NULL AND sync_uuid = $3))`, [schoolId, created.id_competencia, created.sync_uuid]);
             ...
             WHERE c.id_colegio = $1
               ...
               AND c.id_periodo != $5
               AND ea.id_evidencia_dba = ANY($6::int[])`, [schoolId, created.id_anio, created.id_materia, created.id_grupo, periodId, idEvidenciasDba]);
             ...
             WHERE id_evidencia_dba = ANY($1::int[]) AND estado = 'ACTIVO'`, [idEvidenciasDba]);
                    if (officialEvsRes.rows.length > 0) {
                        ...
                        await client.query(`DELETE FROM evidencia_aprendizaje 
               WHERE id_competencia = ANY($1::int[]) AND id_evidencia_dba IS NULL`, [sisterCompIds]);
...
const updateAcademicPeriodPercentage = async (req, res) => {
    ...
    try {
        ...
        const periodRes = await client.query(`SELECT id_periodo, nombre, estado, porcentaje, mes_inicio, dia_inicio, mes_fin, dia_fin, id_anio
       FROM periodo_academico
       WHERE id_periodo = $1 AND id_colegio = $2`, [periodId, schoolId]);
        if (periodRes.rows.length === 0) {
            await client.query("ROLLBACK");
        ...
        const yearRes = await client.query(`SELECT tipo_calendario FROM anio_lectivo WHERE id_anio = $1 AND id_colegio = $2`, [period.id_anio, schoolId]);
       ...
       FROM periodo_academico
       WHERE id_colegio = $1 AND id_anio = $2 AND id_periodo != $3`, [schoolId, period.id_anio, periodId]);
        const getNormalizedDateVal = (month, day, calType) => {
            if (calType === 'B') {
                const normalizeMonth = (m) => m >= 8 ? m - 7 : m + 5;
                return normalizeMonth(month) * 100 + day;
        ...
        if (period.estado === 'PENDIENTE') {
         ...
         LIMIT 1`, [schoolId, period.id_anio, periodId]);
            if (activePeriodRes.rows.length > 0) {
                const active = activePeriodRes.rows[0];
                if (active.mes_fin && active.dia_fin) {
                    const activeEndVal = getNormalizedDateVal(active.mes_fin, active.dia_fin, calendarType);
       ...
       RETURNING id_periodo, nombre, estado, porcentaje, mes_inicio, dia_inicio, mes_fin, dia_fin, id_anio`, [porcentaje, mesInicio, diaInicio, mesFin, diaFin, periodId, schoolId]);
        // Record in audit
        if (activeAuditoriaId) {
            const valorAntiguo = {
...
const approveAcademicPeriod = async (req, res) => {
    ...
    try {
        await client.query("BEGIN");
        // 1. Get current period
        const periodRes = await client.query(`SELECT id_periodo, nombre, estado, id_anio, trimestre
       FROM periodo_academico
       WHERE id_periodo = $1 AND id_colegio = $2`, [periodId, schoolId]);
        if (periodRes.rows.length === 0) {
            await client.query("ROLLBACK");
       ...
       LIMIT 1`, [schoolId, period.id_anio, period.trimestre]);
            ...
            if (trashRes.rows.length > 0) {
                ...
                if (backup.competencies && Array.isArray(backup.competencies)) {
                    for (const comp of backup.competencies) {
                        await client.query('INSERT INTO competencias (descripcion, id_materia, id_periodo, id_anio, id_grupo, id_colegio) VALUES ($1, $2, $3, $4, $5, $6)', [comp.descripcion, newSubjectId, comp.id_periodo, comp.id_anio, comp.id_grupo, schoolId]);
                    }
                }
                // 3. Limpiar papelera
...
const getSubjectCurriculumDetails = async (req, res) => {
    try {
        const subjectId = Number(req.params.id);
        const schoolId = parseSchoolId(req.query.schoolId);
        const yearId = req.query.yearId ? Number(req.query.yearId) : null;
        ...
        // 1. Materia info
        const subRes = await db_1.pool.query("SELECT * FROM materias WHERE id_materia = $1 AND id_colegio = $2", [subjectId, schoolId]);
       ...
       ORDER BY p.id_periodo ASC, tg.nombre ASC, ne.nombre ASC, sec.nombre ASC`, [subjectId, activeYear.id_anio, schoolId]);
        ...
        let evidences = [];
        if (compIds.length > 0) {
            const evRes = await db_1.pool.query(`SELECT ea.id_evidencia, ea.id_competencia, ea.descripcion, ea.orden, ea.id_evidencia_dba,
       ...
       JOIN jornada j ON g.id_jornada = j.id_jornada
       WHERE g.id_colegio = $1
       ORDER BY tg.nombre ASC, ne.nombre ASC, sec.nombre ASC`, [schoolId]);
        res.json({
            subject,
            activeYear,
            periods,
...
const checkCompetenciaUsage = async (req, res) => {
    ...
    try {
        const compRes = await db_1.pool.query(`SELECT c.id_competencia, c.sync_uuid, c.descripcion
       FROM competencias c
       WHERE c.id_competencia = $1 AND c.id_colegio = $2`, [competencyId, schoolId]);
        if (compRes.rows.length === 0) {
            res.status(404).json({ error: "Competencia no encontrada" });
            return;
        }
        const { sync_uuid, descripcion } = compRes.rows[0];
        const usageRes = await db_1.pool.query(`SELECT 
       ...
       GROUP BY u.nombre, u.apellido, m.nombre, ne.nombre, tg.nombre, s.nombre`, [schoolId, competencyId, sync_uuid || null]);
        res.json({
...
exports.checkCompetenciaUsage = checkCompetenciaUsage;
const deleteCompetencyByAdmin = async (req, res) => {
    const competencyId = Number(req.params.id);
    const schoolId = parseSchoolId(req.query.schoolId);
    if (!competencyId || !schoolId) {
        res.status(400).json({ error: "ID de competencia y colegio son obligatorios" });
    ...
    try {
        await client.query("BEGIN");
        const check = await client.query(`SELECT c.id_competencia, c.sync_uuid, p.estado AS period_estado 
       FROM competencias c
       JOIN periodo_academico p ON p.id_periodo = c.id_periodo
       WHERE c.id_competencia = $1 AND c.id_colegio = $2`, [competencyId, schoolId]);
        ...
        const targetCompIdsRes = await client.query(`SELECT id_competencia FROM competencias 
       WHERE id_colegio = $1 AND (id_competencia = $2 OR (sync_uuid IS NOT NULL AND sync_uuid = $3))`, [schoolId, competencyId, sync_uuid]);
        ...
        if (compIds.length > 0) {
         ...
         WHERE id_competencia = ANY($1::int[])`, [compIds]);
            ...
            await client.query(`UPDATE actividad_materia SET id_competencia = NULL WHERE id_competencia = ANY($1::int[])`, [compIds]);
            // 3. Delete competencies
            await client.query(`DELETE FROM competencias WHERE id_competencia = ANY($1::int[]) AND id_colegio = $2`, [compIds, schoolId]);
        }
        await client.query("COMMIT");
...
exports.deleteCompetencyByAdmin = deleteCompetencyByAdmin;
    ...
    try {
        ...
        if (!targetPeriodId) {
            ...
            if (activePeriodRes.rows.length > 0) {
                ...
                const lastPeriodRes = await db_1.pool.query(`SELECT id_periodo FROM periodo_academico WHERE id_colegio = $1 AND id_anio = $2 ORDER BY id_periodo DESC LIMIT 1`, [schoolId, targetYearId]);
        ...
        const [studentsCountRes, teachersCountRes, disciplinaryRes, desertionRes, studentsByGradeRes, teachersByGradeRes, disciplinaryByGradeRes, desertionByGradeRes] = await Promise.all([
            db_1.pool.query(`SELECT COUNT(*) as total FROM matricula WHERE id_colegio = $1 AND id_anio = $2 AND estado = 'ACTIVA'`, [schoolId, targetYearId]),
         ...
         WHERE id_colegio = $1 AND tipo = 'DISCIPLINARIA' ${targetPeriodId ? "AND id_periodo = $2" : ""}`, targetPeriodId ? [schoolId, targetPeriodId] : [schoolId]),
            db_1.pool.query(`SELECT COUNT(*) as total FROM matricula WHERE id_colegio = $1 AND id_anio = $2 AND estado = 'CANCELADA'`, [schoolId, targetYearId]),
...
const saveEnrollmentConfig = async (req, res) => {
    ...
    try {
       ...
       RETURNING *`, [id_colegio, id_anio, fecha_inicio, fecha_cierre, habilitada !== undefined ? Boolean(habilitada) : true]);
...
exports.createExtraordinaryEnrollment = createExtraordinaryEnrollment;
const approveExtraordinaryEnrollment = async (req, res) => {
    const authReq = req;
    const { id } = req.params;
    const { motivo_cambio } = req.body;
    ...
    try {
        ...
        const updatedRes = await client.query("UPDATE matricula SET estado = 'APROBADA' WHERE id_matricula = $1 RETURNING *", [id]);
        const updatedMat = updatedRes.rows[0];
        // Notification: send email to parent with tracking token
        await notificationService_1.NotificationService.sendExtraordinaryApprovalEmail(mat.correo_padre, 'Acudiente', mat.token_seguimiento);
...
exports.approveExtraordinaryEnrollment = approveExtraordinaryEnrollment;
const rejectExtraordinaryEnrollment = async (req, res) => {
    const authReq = req;
    const { id } = req.params;
    const { motivo_cambio } = req.body;
    const schoolId = authReq.user?.schoolId;
...
exports.rejectExtraordinaryEnrollment = rejectExtraordinaryEnrollment;
const createReingresoEnrollment = async (req, res) => {
    const authReq = req;
    const schoolId = authReq.user?.schoolId;
    if (!schoolId) {
        res.status(400).json({ error: "No se pudo identificar el colegio del directivo" });
    ...
    const { id_estudiante, id_nivel, id_grupo, id_anio, motivo, observaciones, tiene_discapacidad, es_extranjero, motivo_cambio } = req.body;
    ...
    try {
        ...
        const existingEnrollmentRes = await client.query(`SELECT id_matricula, estado FROM matricula 
       WHERE id_estudiante = $1 AND id_colegio = $2 AND id_anio = $3 AND estado IN ('ACTIVA', 'TRASLADADA', 'PENDIENTE', 'CORRECCION')`, [id_estudiante, schoolId, id_anio]);
        ...
        const parentRes = await client.query(`SELECT u.email FROM detalle_padrefamilia dp
       ...
       LIMIT 1`, [id_estudiante, schoolId]);
...
exports.createReingresoEnrollment = createReingresoEnrollment;
const approveReingresoEnrollment = async (req, res) => {
    const authReq = req;
    const { id } = req.params;
    const { motivo_cambio } = req.body;
    const schoolId = authReq.user?.schoolId;
    ...
    try {
        ...
        await notificationService_1.NotificationService.sendReingresoApprovalEmail(mat.correo_padre, 'Acudiente', mat.token_seguimiento);
        // Supervision Logging if admin_general
        const isSupervised = authReq.user?.roles.includes("admin_general");
        if (isSupervised) {
            const auditRes = await client.query(`SELECT id_auditoria FROM auditoria_supervision 
...
exports.approveReingresoEnrollment = approveReingresoEnrollment;
...
// ─────────────────────────────────────────────────────────────────────────────
const renameSingleCourse = async (req, res) => {
    ...
    if (nombre.length > 10) {
        res.status(400).json({ error: "El nombre no puede superar los 10 caracteres" });
        return;
    }
    const client = await db_1.pool.connect();
    try {
        ...
        const groupRes = await client.query(`SELECT g.id_grupo, g.id_seccion FROM grupos g WHERE g.id_grupo = $1 AND g.id_colegio = $2`, [idGrupo, schoolId]);
...
const bulkRenameCourses = async (req, res) => {
    ...
    if (!base) {
        res.status(400).json({ error: "El prefijo no puede estar vacío" });
        return;
    }
    if (base.length > 10) {
        res.status(400).json({ error: "El prefijo no puede superar los 10 caracteres" });
        return;
    }
    ...
    try {
        ...
        const gtRes = await client.query(`SELECT tg.id_tipo_grado 
       FROM tipo_grado tg
       JOIN nivel_escolar ne ON tg.id_nivel = ne.id_nivel
       WHERE tg.id_tipo_grado = $1 AND ne.id_colegio = $2`, [idTipoGrado, schoolId]);
        ...
        const groupsRes = await client.query(`SELECT id_grupo, id_seccion FROM grupos WHERE id_tipo_grado = $1 AND id_colegio = $2 ORDER BY id_grupo ASC`, [idTipoGrado, schoolId]);
        const groups = groupsRes.rows;
        if (!groups.length) {
            res.status(400).json({ error: "Este grado no tiene cursos" });
            await client.query("ROLLBACK");
            return;
        }
        ...
        for (let i = 0; i < groups.length; i++) {
            ...
            if (shared <= 1) {
                ...
                const newSecRes = await client.query(`INSERT INTO secciones (nombre) VALUES ($1) RETURNING id_seccion`, [nuevoNombre]);
                const newSeccionId = newSecRes.rows[0].id_seccion;
...
exports.bulkRenameCourses = bulkRenameCourses;
// ============================================================================
// ─── Planeación y Gestión de DBA en Colegios (Fase 2) ────────────────────────
// ============================================================================
const getDbaPlaneacionDisponibles = async (req, res) => {
    const schoolId = parseSchoolId(req.params.schoolId);
    ...
    try {
        ...
        const gradeRes = await db_1.pool.query(`SELECT tg.nombre 
       FROM grupos g
       JOIN tipo_grado tg ON tg.id_tipo_grado = g.id_tipo_grado
       WHERE g.id_grupo = $1`, [groupId]);
       ...
       WHERE cvc.id_colegio = $1
         ...
         AND cvc.grado = $3`, [schoolId, subjectId, gradeName]);
        ...
        const dbaRes = await db_1.pool.query(`SELECT d.id_dba, d.numero_dba, d.enunciado, d.area, d.grado, d.version_curricular,
              COALESCE(
                (SELECT json_agg(
                   json_build_object(
       ...
       ORDER BY d.numero_dba`, [subjectId, versionCurricular, gradeName]);
        ...
        if (groupId && subjectId) {
           ...
           WHERE c.id_colegio = $1
             ...
             AND ea.id_evidencia_dba IS NOT NULL`, [schoolId, competencyId]);
                for (const row of ownRes.rows) {
                    ownAssignedSet.add(Number(row.id_evidencia_dba));
                }
                // Evidencias asociadas a OTRAS competencias
                const otherRes = await db_1.pool.query(`SELECT DISTINCT ea.id_evidencia_dba, c.descripcion AS competencia_descripcion, p.nombre AS periodo_nombre
           FROM evidencia_aprendizaje ea
           JOIN competencias c ON c.id_competencia = ea.id_competencia
           JOIN periodo_academico p ON p.id_periodo = c.id_periodo
           WHERE c.id_colegio = $1
             ...
             AND ea.id_evidencia_dba IS NOT NULL`, [schoolId, subjectId, groupId, competencyId]);
                ...
                const assignedRes = await db_1.pool.query(`SELECT DISTINCT ea.id_evidencia_dba, c.descripcion AS competencia_descripcion, p.nombre AS periodo_nombre
           FROM evidencia_aprendizaje ea
           JOIN competencias c ON c.id_competencia = ea.id_competencia
           JOIN periodo_academico p ON p.id_periodo = c.id_periodo
           WHERE c.id_colegio = $1
             ...
             AND c.id_anio = (SELECT id_anio FROM anio_lectivo WHERE id_colegio = $1 ORDER BY id_anio DESC LIMIT 1)
             AND ea.id_evidencia_dba IS NOT NULL`, [schoolId, subjectId, groupId]);
...
exports.getDbaPlaneacionDisponibles = getDbaPlaneacionDisponibles;
const vincularEvidenciasDbaACompetencia = async (req, res) => {
    const competencyId = Number(req.params.competenciaId);
    const schoolId = parseSchoolId(req.body.schoolId);
    const idEvidenciasDba = req.body.id_evidencias_dba; // Arreglo de IDs de evidencias_dba
    ...
    try {
        ...
        const compRes = await client.query(`SELECT id_competencia, id_anio, id_grupo, id_materia, id_periodo, id_colegio, sync_uuid 
       FROM competencias 
       ...
       WHERE g1.id_grupo = $1 AND g1.id_colegio = $2`, [comp.id_grupo, schoolId]);
        ...
        if (idEvidenciasDba.length > 0) {
         ...
         WHERE c.id_colegio = $1
           ...
           AND ea.id_evidencia_dba = ANY($6::int[])`, [schoolId, comp.id_anio, comp.id_materia, peerGroupIds, comp.sync_uuid, idEvidenciasDba]);
            if (alreadyAssignedRes.rows.length > 0) {
                await client.query("ROLLBACK");
                const names = alreadyAssignedRes.rows.map(r => `Evidencia ID ${r.id_evidencia_dba} en periodo ${r.periodo_nombre}`).join(", ");
                res.status(400).json({ error: `Una o más evidencias ya están asignadas a otra competencia: ${names}` });
       ...
       WHERE id_competencia = ANY($1::int[]) AND id_evidencia_dba IS NULL`, [sisterCompIds]);
        ...
        for (const targetCompId of sisterCompIds) {
            ...
            const existingRes = await client.query(`SELECT id_evidencia, id_evidencia_dba FROM evidencia_aprendizaje 
         WHERE id_competencia = $1 AND id_evidencia_dba IS NOT NULL`, [targetCompId]);
            const existingMap = new Map(); // id_evidencia_dba -> id_evidencia
            existingRes.rows.forEach(r => existingMap.set(r.id_evidencia_dba, r.id_evidencia));
            const activeDbaIds = officialEvsRes.rows.map(r => r.id_evidencia_dba);
