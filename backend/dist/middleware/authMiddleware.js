               ...
               WHERE d.id_colegio = $1 AND d.estado = 'ACTIVO'`, [supervision.id_colegio]);
                 ...
                 VALUES ($1, $2, 'SALIDA', $3)`, [
                    ...
                    if (req.method === 'GET') {
                        ...
                        if (auditDetails) {
                            db_1.pool.query(`INSERT INTO auditoria_acciones_realizadas
...
function getModuloFromUrl(url) {
    ...
    if (url.startsWith('/api/boletines'))
        return 'BOLETINES';
    if (url.startsWith('/api/academic-admin'))
        return 'CONFIGURACION';
    if (url.startsWith('/api/matriculas') || url.startsWith('/api/matricula'))
        return 'MATRICULAS';
    if (url.startsWith('/api/teacher'))
        return 'DOCENTES';
    if (url.startsWith('/api/grados'))
        return 'GRADOS';
    if (url.startsWith('/api/dba'))
        return 'DBA';
    if (url.startsWith('/api/support'))
        return 'SOPORTE';
    return 'GENERAL';
}
function getTipoAccionFromMethod(method) {
    if (method === 'POST')
        return 'CREACION';
    if (method === 'DELETE')
        return 'ELIMINACION';
...
function getAccionPrefixFromMethod(method) {
    if (method === 'POST')
