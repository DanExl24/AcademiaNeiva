...
async function run() {
    ...
    `);
        ...
        for (const role of roles) {
            const res = await client.query(`INSERT INTO rol (nombre) VALUES ($1) RETURNING id_rol`, [role]);
       ...
       VALUES ($1, $2, $3, $4, NULL, true, 'ACTIVO', 3, '1000000000', '3000000000') RETURNING id_usuario`, [adminGeneralEmail, adminGeneralHash, "Admin", "General"]);
    ...
    `);
        ...
        for (const school of schools) {
            ...
            await client.query(`INSERT INTO colegio (id_colegio, nombre, tipo_colegio, sede, contacto, correo, dane, tipo_calendario)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [school.id, school.nombre, school.tipo, school.sede, school.contacto, school.correo, school.dane, school.tipo_calendario]);
         ...
         VALUES ($1, 0, 5, 3, 'AUTOMATICO')`, [school.id]);
            ...
            const rectorRes = await client.query(`INSERT INTO usuario (email, password, nombre, apellido, id_colegio, activo, estado, id_tipodocumento, documento, telefono)
         VALUES ($1, $2, $3, $4, $5, true, 'ACTIVO', 3, $6, $7) RETURNING id_usuario`, [rectorEmail, directivoHash, "Rector", school.nombre, school.id, `10010000${school.id}`, String(school.contacto)]);
            ...
            const directivoResult = await client.query(`INSERT INTO usuario (email, password, nombre, apellido, id_colegio, activo, estado, id_tipodocumento, documento, telefono)
         VALUES ($1, $2, $3, $4, $5, true, 'ACTIVO', 3, $6, $7) RETURNING id_usuario`, [directivoEmail, directivoHash, "Directivo", school.nombre, school.id, `10020000${school.id}`, String(school.contacto)]);
