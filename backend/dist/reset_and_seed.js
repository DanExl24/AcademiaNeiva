...
async function run() {
    ...
    `);
      ...
      (1, 'A', 1)
    `);
        const passwordHash = await bcrypt_1.default.hash('docente123', 10);
        const directivoHash = await bcrypt_1.default.hash('directivo123', 10);
        for (let i = 0; i < colegios.length; i++) {
            const [nombre, tipo, sede, contacto, correo, dane, domain] = colegios[i];
            const cId = i + 1;
            // Crear Directivo de prueba para este colegio
            const dirEmail = `directivo@${domain}`;
            const dirUser = await client.query(`INSERT INTO usuario (email, password, nombre, id_colegio) VALUES ($1, $2, $3, $4) RETURNING id_usuario`, [dirEmail, directivoHash, `Directivo ${nombre}`, cId]);
            ...
            const docUser = await client.query(`INSERT INTO usuario (email, password, nombre, id_colegio) VALUES ($1, $2, $3, $4) RETURNING id_usuario`, [docEmail, passwordHash, `Docente ${nombre}`, cId]);
            ...
            await client.query(`INSERT INTO docente (id_docente, nombre, apellido, documento, id_tipodocumento, id_colegio, id_usuario) 
         VALUES ($1, $2, $3, $4, $5, $6, $7)`, [i + 1, `Docente`, nombre, `DOC-${i + 1}`, 3, cId, idUsuarioDocente]);
        }
    ...
    `);
        ...
        for (let cId = 1; cId <= 5; cId++) {
            ...
            for (const mNombre of materiasNombres) {
                await client.query(`INSERT INTO materias (id_materia, nombre, id_colegio) VALUES ($1, $2, $3)`, [materiaId, mNombre, cId]);
                ...
                for (const g of gradosColegio) {
                    await client.query(`INSERT INTO detalle_grados (id_detallegrado, id_grado, id_materia, id_docente, id_colegio) 
             VALUES ($1, $2, $3, $4, $5)`, [detalleGradoId++, g.id, materiaId, idDocente, cId]);
                }
                materiaId++;
            }
        }
