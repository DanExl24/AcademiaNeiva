...
class MatriculaService {
    ...
    static async getDetails(idMatricula) {
       ...
       WHERE m.id_matricula = $1`, [idMatricula]);
        ...
        const sections = await db_1.pool.query(`SELECT g.id_grupo as id_grado, s.nombre as seccion, 
              (g.cupos_totales - (SELECT COUNT(*) FROM matricula WHERE id_grupo = g.id_grupo AND estado IN ('ACTIVA', 'TRASLADADA'))) as cupos_restantes
       FROM grupos g
       ...
       WHERE g.id_colegio = $1 AND ne.nombre = $2 AND tg.nombre = $3 AND g.id_jornada = $4`, [mat.id_colegio, mat.grado_nivel, mat.tipo_grado, mat.id_jornada]);
        ...
        if (mat.correo_padre) {
            const existingUserRes = await db_1.pool.query(`SELECT u.id_usuario, u.nombre, u.apellido, u.email,
                ARRAY_AGG(r.nombre ORDER BY r.nombre) as roles
         FROM usuario u
         JOIN usuario_rol ur ON u.id_usuario = ur.id_usuario
         JOIN rol r ON ur.id_rol = r.id_rol
         WHERE u.email = $1
         GROUP BY u.id_usuario, u.nombre, u.apellido, u.email`, [mat.correo_padre]);
    ...
    static async assignGrade(idMatricula, idGrado) {
        ...
        await db_1.pool.query(`UPDATE matricula SET id_grupo = $1 WHERE id_matricula = $2`, [idGrado, idMatricula]);
        return { success: true };
    }
    ...
    static async finalizeEnrollment(idMatricula, data) {
        ...
        try {
            ...
            // Registro Estudiante
            const studentRes = await client.query(`INSERT INTO estudiante (nombre, apellido, documento, codigo, id_tipodocumento, id_nivel, id_colegio, id_usuario)
    ...
    static async toggleTransferStatus(idMatricula, esTraslado) {
        await db_1.pool.query('UPDATE matricula SET es_traslado = $1 WHERE id_matricula = $2', [esTraslado, idMatricula]);
        return { success: true };
    }
