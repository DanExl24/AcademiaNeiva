...
class NotificationService {
    ...
    static async sendExtraordinaryApprovalEmail(to, parentName, token) {
        if (!to) {
    ...
    static async sendReingresoApprovalEmail(to, parentName, token) {
        if (!to) {
    ...
    static async sendEnrollmentSubmittedEmail(to, parentName, studentName, trackingToken) {
        if (!to) {
    ...
    static async sendEnrollmentEmailVerificationCode(to, code) {
        if (!to)
            return;
        const html = `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #1f2937;">
