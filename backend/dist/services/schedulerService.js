...
const adminGeneralNotificationService_1 = require("./adminGeneralNotificationService");
class SchedulerService {
     ...
     */
    static async activatePendingPeriods() {
            ...
            for (const pa of pendingPeriods.rows) {
               ...
               WHERE id_colegio = $1 AND id_anio = $2 AND trimestre = $3`, [pa.id_colegio, pa.id_anio, pa.trimestre - 1]);
                    ...
                    if (canActivate) {
                        ...
                        await client.query("UPDATE periodo_academico SET estado = 'ABIERTO' WHERE id_periodo = $1", [pa.id_periodo]);
     ...
     */
    static async expireSupervisions() {
        const client = await db_1.pool.connect();
        try {
            // Consultar supervisiones activas expiradas
            const expiredSupervisions = await client.query(`SELECT a.*, c.nombre AS colegio_nombre, u.email AS admin_email, u.nombre AS admin_firstname, u.apellido AS admin_lastname
         ...
         WHERE a.estado_supervision = 'ACTIVA' 
           ...
           SET estado_supervision = 'EXPIRADA',
               fecha_salida = NOW()
           WHERE id_auditoria = $1`, [aud.id_auditoria]);
                // 2. Contar acciones realizadas durante esta supervisión
                const accionesRes = await client.query('SELECT COUNT(*)::int AS total FROM auditoria_acciones_realizadas WHERE id_auditoria = $1', [aud.id_auditoria]);
           ...
           WHERE d.id_colegio = $1 AND d.estado = 'ACTIVO'`, [aud.id_colegio]);
             ...
             VALUES ($1, $2, 'SALIDA', $3)`, [
                    ...
                    adminGeneralNotificationService_1.AdminGeneralNotificationService.sendSupervisionFinalizada(dir.email, `${dir.nombre} ${dir.apellido || ''}`.trim(), aud.admin_email, aud.colegio_nombre, `${duracionStr} (Expiración automática)`, totalAcciones).catch(err => console.error(`Error enviando correo de expiración de supervisión al directivo ${dir.email}:`, err));
                }
                await client.query('COMMIT');
            }
        }
