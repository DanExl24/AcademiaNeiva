...
<template>
  <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-3xl font-black text-slate-900">Control de Cierre de Periodo</h1>
        <p class="mt-1 text-slate-500">Supervisa qué docentes han finalizado la carga académica y ejecuta cierres formales de notas.</p>
      </div>
    </div>
    ...
    <template v-else>
      <div class="rounded-3xl border border-slate-100 bg-white shadow-sm overflow-hidden">
        <div class="border-b border-slate-100 p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            <div class="rounded-2xl bg-rose-50 p-3 text-rose-600">
              <Lock class="h-6 w-6" />
            </div>
            <div>
              <h2 class="text-lg font-black text-slate-900">Seleccionar periodo</h2>
              <p class="text-sm text-slate-500">Puedes ver estados y forzar el cierre del periodo en curso.</p>
            </div>
          </div>
          <select 
            v-model="selectedPeriodId"
            class="min-w-[200px] rounded-2xl border border-slate-200 bg-slate-50 p-4 font-black outline-none focus:border-rose-300 focus:ring-4 focus:ring-rose-500/10 transition-shadow"
          >
        ...
        <div v-if="selectedPeriodId && !detailsLoading" class="p-6 md:p-8 bg-slate-50/50">
          ...
          <div v-if="periodDetails" class="mb-8 flex flex-col lg:flex-row gap-6 justify-between items-start lg:items-center">
            <div class="flex gap-4">
              <div class="flex flex-col rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
                <span class="text-[11px] font-black uppercase tracking-widest text-slate-400">Total asignaciones</span>
                <span class="text-2xl font-black text-slate-900">{{ teachers.reduce((sum, t) => sum + t.total_asignaciones, 0) }}</span>
              </div>
              <div class="flex flex-col rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 shadow-sm text-rose-900">
                <span class="text-[11px] font-black uppercase tracking-widest text-rose-500">Módulos cerrados</span>
                <span class="text-2xl font-black text-rose-700">{{ totalClosed }}</span>
              </div>
              <div class="flex flex-col rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 shadow-sm text-amber-900">
                <span class="text-[11px] font-black uppercase tracking-widest text-amber-500">Faltantes</span>
                <span class="text-2xl font-black text-amber-700">{{ totalPending }}</span>
              </div>
            </div>
            ...
            <div class="flex items-center gap-4">
              <div class="flex items-center gap-2 mr-2">
                ...
                <span class="h-3 w-3 rounded-full bg-slate-400" v-else></span>
                <span class="text-sm font-black tracking-wide" :class="periodDetails.estado === 'ABIERTO' ? 'text-emerald-700' : 'text-slate-700'">PERIODO {{ periodDetails.estado }}</span>
              </div>
              ...
              <template v-else>
                <div class="px-5 py-3 bg-slate-100 rounded-2xl text-slate-500 font-bold text-sm flex gap-2 items-center border border-slate-200">
          ...
          <div v-if="teachers.length === 0" class="text-center py-20 opacity-60">
            ...
            <h3 class="text-lg font-bold text-slate-600">Sin carga académica</h3>
            <p class="text-slate-500 text-sm mt-1">No hay docentes asignados en este periodo.</p>
          </div>

          <div v-else class="space-y-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <h2 class="text-lg font-black text-slate-900 px-1">Progreso por Docente</h2>
              
              <div class="flex flex-col sm:flex-row items-center gap-3">
                <div class="flex bg-slate-100/80 p-1 rounded-2xl w-full sm:w-auto">
                  <button 
                    @click="filterStatus = 'TODOS'"
                    class="px-4 py-2 rounded-xl text-[13px] font-bold transition-all w-full sm:w-auto"
                    :class="filterStatus === 'TODOS' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'"
                  >Todos</button>
                  <button 
                    @click="filterStatus = 'PENDIENTE'"
                    class="px-4 py-2 rounded-xl text-[13px] font-bold transition-all w-full sm:w-auto"
                    :class="filterStatus === 'PENDIENTE' ? 'bg-amber-500 text-white shadow-sm' : 'text-amber-600/70 hover:text-amber-700'"
                  >Pendientes</button>
                  <button 
                    @click="filterStatus = 'CERRADO'"
                    class="px-4 py-2 rounded-xl text-[13px] font-bold transition-all w-full sm:w-auto"
                    :class="filterStatus === 'CERRADO' ? 'bg-emerald-500 text-white shadow-sm' : 'text-emerald-600/70 hover:text-emerald-700'"
                  >Cerrados</button>
                </div>

                <div class="flex items-center gap-3 bg-white border border-slate-200 rounded-2xl px-4 py-2 hover:border-slate-300 transition-colors focus-within:ring-4 focus-within:ring-rose-500/10 focus-within:border-rose-300 w-full md:w-80">
                  <Search class="w-4 h-4 text-slate-400 shrink-0" />
                  <input 
                    v-model="searchQuery" 
                    type="text" 
                    placeholder="Buscar docente o materia..."
                    class="bg-transparent border-none outline-none w-full text-sm font-semibold text-slate-700 placeholder:text-slate-400 py-1"
                  />
                </div>
              </div>
            </div>
            
            <div v-if="filteredTeachers.length === 0" class="text-center py-16 bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
              <Search class="w-12 h-12 text-slate-200 mx-auto mb-4" />
              <h3 class="text-lg font-bold text-slate-600">No se encontraron resultados</h3>
              <p class="text-slate-500 text-sm mt-1">Intenta ajustando tu término de búsqueda. "{{ searchQuery }}"</p>
            </div>
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div 
                ...
                class="rounded-[24px] border bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 overflow-hidden flex flex-col"
                :class="teacher.cerradas === teacher.total_asignaciones ? 'border-emerald-100' : (teacher.cerradas === 0 ? 'border-rose-100/60' : 'border-amber-100/60')"
              >
                <div class="p-5 border-b border-slate-100/60 cursor-pointer select-none group" 
                     :class="teacher.cerradas === teacher.total_asignaciones ? 'bg-emerald-50/40 hover:bg-emerald-50/80' : 'bg-white hover:bg-slate-50/50'"
                     @click="toggleTeacher(teacher.id_docente)"
                >
                  <div class="flex items-start justify-between mb-3">
                    <div class="overflow-hidden pr-3">
                      <h3 class="font-black text-slate-900 border-b border-transparent truncate text-base group-hover:text-rose-600 transition-colors" :title="teacher.docente_nombre">
                        {{ teacher.docente_nombre.split(' ')[0] }} {{ teacher.docente_nombre.split(' ')[2] || teacher.docente_nombre.split(' ')[1] }}
                      </h3>
                      <p class="text-xs text-slate-500 mt-1 truncate font-medium">{{ teacher.docente_email }}</p>
                    </div>
                    <div class="flex items-center gap-3">
                      <div 
                        class="rounded-xl px-2.5 py-1 text-[11px] font-black shrink-0 shadow-sm"
                        :class="[
                          teacher.cerradas === teacher.total_asignaciones ? 'bg-emerald-100 text-emerald-700' : 
                          (teacher.cerradas === 0 ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700')
                        ]"
                      >
                        {{ filterStatus !== 'TODOS' ? teacher.visibleAsigCount : teacher.cerradas + '/' + teacher.total_asignaciones }}
                      </div>
                      <div class="w-8 h-8 rounded-full bg-white flex items-center justify-center border border-slate-100 shadow-sm transition-transform duration-300 group-hover:border-rose-200 group-hover:text-rose-600" :class="{'rotate-180 bg-rose-50 border-rose-200 text-rose-600': expandedTeachers.has(teacher.id_docente)}">
                        <ChevronDown class="w-4 h-4 text-slate-400 group-hover:text-rose-500 transition-colors" />
                      </div>
                    </div>
                  </div>
                  <!-- Progress Bar -->
                  <div v-if="filterStatus === 'TODOS'" class="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                ...
                <div v-show="expandedTeachers.has(teacher.id_docente)" class="p-4 bg-slate-50/50 flex-1 flex flex-col gap-3 border-t border-slate-100/50">
                  <div 
                    v-for="materia in teacher.filteredMaterias" 
                    :key="materia.nombre"
                    class="bg-white rounded-[16px] shadow-sm ring-1 ring-slate-100 overflow-hidden text-sm transition-all hover:ring-slate-200"
                  >
                    <div class="bg-slate-50/80 px-3.5 py-2.5 font-bold text-slate-700 border-b border-slate-100/80 text-[13px] flex items-center gap-2">
                    ...
                    <div class="p-1.5 flex flex-col gap-0.5">
                      <div 
                        ...
                        class="flex items-center justify-between px-3 py-2.5 hover:bg-slate-50 rounded-xl transition-colors group"
                      >
                        <div class="flex items-center gap-3 overflow-hidden">
                          <div class="rounded-full bg-white p-0.5 shadow-sm">
    ...
    <div v-if="forceCloseModal" class="fixed inset-0 z-[100] flex min-h-screen w-screen items-center justify-center bg-slate-950/88 p-4 backdrop-blur-md">
      <div class="w-full max-w-2xl rounded-[28px] bg-white shadow-2xl">
        <div class="border-b border-slate-100 px-6 py-5 md:px-8">
          <div class="flex gap-3 items-center text-rose-600 mb-2">
            <AlertCircle class="w-8 h-8" />
            <h2 class="text-2xl font-black">Cierre Forzado Requerido</h2>
          </div>
        ...
        <div class="px-6 py-6 md:px-8 md:py-8">
          ...
          <div class="rounded-3xl border border-rose-100 bg-rose-50 p-5 mb-6">
          ...
          <div class="max-h-60 overflow-y-auto pr-2 space-y-3">
            <div v-for="item in closePeriodPending" :key="item.id_detallegrado" class="rounded-2xl bg-white border border-slate-200 px-4 py-3 text-sm flex flex-col md:flex-row md:items-center justify-between gap-2 shadow-sm">
              <div>
                <p class="font-black text-slate-700">{{ item.materia_nombre }}</p>
                <p class="text-xs text-slate-500 font-semibold">{{ item.tipo_grado_nombre }} {{ item.seccion_nombre }} · {{ item.jornada_nombre }}</p>
              </div>
              <span class="inline-flex bg-amber-100 text-amber-700 rounded-lg px-2 py-1 text-[11px] font-black uppercase tracking-wider">Pendiente</span>
            </div>
          </div>

          <div class="mt-8 flex flex-col gap-3 md:flex-row md:justify-end">
            <button type="button" @click="forceCloseModal = false" class="rounded-2xl border border-slate-200 px-6 py-4 text-sm font-black text-slate-700 transition hover:bg-slate-50">
              Cancelar
            </button>
