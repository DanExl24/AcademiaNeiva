...
const existingCombinations = ref<{ area: string, version_curricular: string }[]>([])
const importOverwrite = ref(false)

const isExistingDba = computed(() => {
  const area = importForm.value.area.trim().toLowerCase()
  const ver = importForm.value.version_curricular.trim().toLowerCase()
  if (!area || !ver) return false
  return existingCombinations.value.some(
    c => c.area.toLowerCase() === area && c.version_curricular.toLowerCase() === ver
  )
})
...
const handleAssignVersion = async () => {
  ...
  try {
    saving.value = true
    const isBulkSchool = assignForm.value.id_colegio === 'TODOS'
    const isBulkArea = assignForm.value.area === 'TODAS'
    const isBulk = isBulkSchool || isBulkArea
