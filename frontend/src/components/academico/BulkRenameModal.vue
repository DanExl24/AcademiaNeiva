...
<template>
  <Teleport to="body">
    <div v-if="show && bulkTarget" class="fixed inset-0 z-[110] flex items-center justify-center p-3 sm:p-4">
          ...
          <div>
            <div class="flex items-center gap-3.5 sm:gap-4 mb-5 sm:mb-6">
              ...
              <div class="min-w-0 flex-1">
                <h3 class="text-lg sm:text-xl font-black text-slate-900 dark:text-white truncate">Renombre Masivo</h3>
                <p class="text-xs sm:text-sm font-medium text-slate-500 truncate">Grado: {{ bulkTarget.nombre }} | {{ bulkCourseCount }} Cursos</p>
              </div>
            </div>
            ...
            <div class="space-y-3.5 sm:space-y-4">
              ...
              <div class="space-y-1.5 sm:space-y-2">
                <label class="text-xs sm:text-sm font-black text-slate-700 dark:text-slate-300 ml-1">Estructura Base (Prefijo)</label>
                <input 
                  v-model="bulkPrefijo" 
                  type="text"
                  maxlength="10"
              ...
              <div class="space-y-1.5 sm:space-y-2">
                <label class="text-xs sm:text-sm font-black text-slate-700 dark:text-slate-300 ml-1">Tipo de Sufijo (Ordinal)</label>
                <div class="flex flex-col sm:flex-row gap-2.5 sm:gap-4">
                  <label class="flex-1 flex items-center justify-between p-3 sm:p-3.5 bg-slate-50 dark:bg-slate-800 rounded-xl sm:rounded-2xl cursor-pointer border-2 border-transparent hover:border-indigo-500/20 transition-all">
                    <span class="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200">Números (1, 2, 3...)</span>
                    <input type="radio" value="NUMERO" v-model="bulkOrdinalType" class="accent-indigo-600 w-4 h-4" />
                  </label>
                  <label class="flex-1 flex items-center justify-between p-3 sm:p-3.5 bg-slate-50 dark:bg-slate-800 rounded-xl sm:rounded-2xl cursor-pointer border-2 border-transparent hover:border-indigo-500/20 transition-all">
                    <span class="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200">Letras (A, B, C...)</span>
              ...
              <!-- Separator option -->
              <div class="space-y-1.5 sm:space-y-2">
                <label class="text-xs sm:text-sm font-black text-slate-700 dark:text-slate-300 ml-1">Separador con el Ordinal</label>
                <select 
                  v-model="bulkSeparador" 
                  class="w-full bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-indigo-500/20 rounded-xl sm:rounded-2xl p-3 sm:p-4 text-xs sm:text-sm font-bold outline-none text-slate-900 dark:text-white appearance-none cursor-pointer transition-all"
                >
              ...
              <!-- Real-time Preview -->
              <div v-if="previewNames.length > 0" class="bg-indigo-50/50 dark:bg-indigo-950/20 p-3.5 sm:p-4 rounded-xl sm:rounded-2xl border border-indigo-100/50 dark:border-indigo-900/50">
                <h4 class="text-[10px] sm:text-xs font-black uppercase text-slate-400 dark:text-slate-500 mb-2">Vista Previa de los Cursos:</h4>
                <div class="flex flex-wrap gap-1.5 sm:gap-2 max-h-24 overflow-y-auto custom-scrollbar">
                  <span 
                    v-for="(pName, idx) in previewNames" 
                    :key="idx"
                    class="bg-indigo-100 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 px-2.5 sm:px-3 py-1 rounded-full text-[11px] sm:text-xs font-black border border-indigo-200/50 dark:border-indigo-900/50"
                  >
                    {{ pName }}
                  </span>
                </div>
              ...
              <p v-if="bulkPrefijoError" class="text-xs font-bold text-red-500 bg-red-50 dark:bg-red-950/30 p-2.5 sm:p-3 rounded-xl border border-red-100 dark:border-red-950">
                {{ bulkPrefijoError }}
              </p>
            </div>
          </div>
